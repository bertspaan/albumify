			...
			function(data) {
				...
				for(var i in albums) {
					...
					m.Album.fromURI(uri, function(album) {
						$('#albums').append('<li><div><a href="' + album.data.uri + '"><img class="spotify-album-image" src="' + album.data.cover + '" /><a/><span class="artist">De Vis</span><span class="title">De Beste CD</span></div></li>');
					});			
				}
			}
		);
